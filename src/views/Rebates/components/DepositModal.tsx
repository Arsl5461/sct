...
const DepositModal: React.FC<DepositModalProps> = ({ max, onConfirm, onDismiss, tokenName = '', token }) => {
  ...
  function getOutAmount() {
    ...
    const outAmount = +val * (toBondPrice / rebateStats.tombPrice * (1 + (rebateStats.bondPremium / 100)) * (token.params.multiplier / 1000000)) // DENOMINATOR = 10^6
  ...
  function formatOutAmount() {
    ...
    return `Receiving: ${outAmount.toFixed(4)} SCT ($${(outAmount * rebateStats.tombPrice * ftmPrice).toFixed(2)})`
  }
  ...
  return (
    <Modal>
      ...
      <StyledMaxText style = {{color: getOutAmount() < rebateStats.tombAvailable ? "black" : "var(--accent)"}}>
        {rebateStats.tombAvailable > 0 ? `${rebateStats.tombAvailable.toFixed(4)} SCT Available` : "Bond Sold Out"}
      </StyledMaxText>
      <ModalActions>
        ...
        <Button color={ "primary"  } variant="contained" disabled = { true } onClick={() => onConfirm(+val)}>
          Confirm
        </Button>
      </ModalActions>
    </Modal>
