...
import tombLogo from '../assets/img/PSHARES1.png';
import tShareLogo from '../assets/img/PSHARES.png';
...
export class TombFinance {
  ...
  constructor(cfg: Configuration) {
    ...
    this.TOMB = new ERC20(deployments.tomb.address, provider, 'SCT');
    this.TSHARE = new ERC20(deployments.tShare.address, provider, 'PSHARES');
    this.TBOND = new ERC20(deployments.tBond.address, provider, 'PBOND');
    this.AVAX = this.externalTokens['WAVAX'];

    // Uniswap V2 Pair
    this.TOMBWFTM_LP = new Contract(externalTokens['TOMB-AVAX-LP'][0], IUniswapV2PairABI, provider);
  ...
  async getTombStat(): Promise<TokenStat> {
    const { SctWavaxGenesisRewardPool, SctAvaxLpSctRewardPool } = this.contracts;
    const supply = await this.TOMB.totalSupply();
    
    const tombRewardPoolSupply = await this.TOMB.balanceOf(SctWavaxGenesisRewardPool.address);
    const tombRewardPoolSupply2 = await this.TOMB.balanceOf(SctAvaxLpSctRewardPool.address);
  ...
  async getLPStat(name: string): Promise<LPStat> {
    ...
    const token0 = name.startsWith('SCT') ? this.TOMB : this.TSHARE;
    const isTomb = name.startsWith('SCT');
  ...
  async getShareStat(): Promise<TokenStat> {
    const { SctAvaxLPPShareRewardPool } = this.contracts;
    const supply = await this.TSHARE.totalSupply();
    const priceInFTM = await this.getTokenPriceFromPancakeswap(this.TSHARE);
    const tombRewardPoolSupply = await this.TSHARE.balanceOf(SctAvaxLPPShareRewardPool.address);
  ...
  async getTombPriceInLastTWAP(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getSctUpdatedPrice();
  }

  async getBondsPurchasable(): Promise<BigNumber> {
    const { Treasury } = this.contracts;
    return Treasury.getBurnableSctLeft();
  }
  ...
  async getPoolAPRs(bank: Bank): Promise<PoolStats> {
    ...
    const stat = bank.earnTokenName === 'SCT' ? await this.getTombStat() : await this.getShareStat();
  ...
  async getTokenPerSecond(
    ...
    if (earnTokenName === 'SCT') {
      if (!contractName.endsWith('SctRewardPool')) {
        const rewardPerSecond = await poolContract.sctPerSecond();
        if (depositTokenName === 'USDT') {
          return rewardPerSecond.mul(3000).div(11000);
        } else if (depositTokenName === 'JOE') {
          return rewardPerSecond.mul(1000).div(11000);
        } else if (depositTokenName === 'WAVAX') {
          return rewardPerSecond.mul(5000).div(11000);
        } else if (depositTokenName === 'USDC') {
          return rewardPerSecond.mul(2000).div(11000);
        }
      ...
      if (Date.now() - startDateTime.getTime() > THREE_DAYS) {
        return await poolContract.epochSctPerSecond(1);
      }
      return await poolContract.epochSctPerSecond(0);
    }
  ...
  async getDepositTokenPriceInDollars(tokenName: string, token: ERC20) {
    ...
    if (tokenName === 'WAVAX') {
      ...
      // } else if (tokenName === "2OMB-WAVAX LP") {
  ...
  async buyBonds(amount: string | number): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const treasuryTombPrice = await Treasury.getSctPrice();
  ...
  async redeemBonds(amount: string): Promise<TransactionResponse> {
    const { Treasury } = this.contracts;
    const priceForTomb = await Treasury.getSctPrice();
  ...
  async earnedFromBank(
    ...
    try {
      if (earnTokenName === 'SCT') {
        return await pool.pendingSCT(poolId, account);
      } else {
  ...
  async getTokenPriceFromSpiritswap(tokenContract: ERC20): Promise<string> {
    ...
    const wftm = new TokenSpirit(chainId, WAVAX.address, WAVAX.decimal);
    const token = new TokenSpirit(chainId, tokenContract.address, tokenContract.decimal, tokenContract.symbol);
    try {
      const wftmToToken = await FetcherSpirit.fetchPairData(wftm, token, this.provider);
      const liquidityToken = wftmToToken.liquidityToken;
      let ftmBalanceInLP = await WAVAX.balanceOf(liquidityToken.address);
      let ftmAmount = Number(getFullDisplayBalance(ftmBalanceInLP, WAVAX.decimal));
  ...
  async getWFTMPriceFromPancakeswap(): Promise<string> {
    ...
    try {
      const fusdt_wftm_lp_pair = this.externalTokens['USDT-AVAX-LP'];
  ...
  async canUserClaimRewardFromMasonry(): Promise<boolean> {
    ...
    const canClaimReward = await Masonry.canClaimReward(this.myAccount);
    ...
    const result = notStaked ? true : canClaimReward;
    return result;
  }
  ...
  async watchAssetInMetamask(assetName: string): Promise<boolean> {
    ...
    if (ethereum && ethereum.networkVersion === config.chainId.toString()) {
      ...
      if (assetName === 'SCT') {
        asset = this.TOMB;
        assetUrl = "/SCT.png";
      } else if (assetName === 'PSHARES') {
        asset = this.TSHARE;
        assetUrl = "/PSHARES.png";
      } else if (assetName === 'PBOND') {
        asset = this.TBOND;
        assetUrl = "/PBOND.png";
      } else if (assetName === 'SCT-WAVAX LP') {
        asset = {
          address: this.config.externalTokens['SCT-WAVAX LP'][0],
          symbol: 'SCT-AVAX'
        };
        assetUrl = '/SCT-WAVAX.png';
      } else if (assetName === 'PSHARES-WAVAX LP') {
        asset = {
          address: this.config.externalTokens['PSHARES-WAVAX LP'][0],
          symbol: 'PSHARE-AVAX'
        };
        assetUrl = '/PSHARES-WAVAX.png';
      } 
